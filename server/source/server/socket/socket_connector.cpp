...
bool SocketServerConnector::createAndBindListeningFD(struct addrinfo *addressInfo){
	...
	for(rp = addressInfo; rp != nullptr; rp = rp->ai_next){
		try{
			listeningFD = createSocket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
		}
		catch(std::runtime_error & ret){
			continue;
		}
		...
		try{
			bindSocket(listeningFD, rp->ai_addr, rp->ai_addrlen);
			addFD(listeningFD);
			return true;
		}
		catch(std::runtime_error & ret){
...
void SocketServerConnector::openListeningPort(){
	...
	listenSocket(listeningFD, SOMAXCONN);
...
void SocketServerConnector::newConnection(){
	while (*running){
		...
		int newConnection = fileDescriptors->addFD(acceptSocket(listeningFD, &in_addr, &in_len, done));
		if (done){
...
void SocketServerConnector::readHandshake(int FD){
	...
	while (*running){
		bool done;
		size_t count = readFD(FD, &buf[0], buffSize-1, done);
		if(done){
