...
bool WebsocketWriteBuffers::writeData(int index){
	std::lock_guard<std::mutex> lck(mut);
	pendingMessage& tempBuff = writeBuffer[index];
	size_t  ret = writeFD(index, &tempBuff.message[tempBuff.begin],  tempBuff.message.size()-tempBuff.begin);
	tempBuff.begin += ret;
...
WebsocketWriteBuffers::WebsocketWriteBuffers() : mut(), writeBuffer(){}
